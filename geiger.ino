...
void setup(void)                       //инициализация
{
    ...
    TCCR1B = 0 b00000101;              //prescaler: 16000000/1024=15625Гц
    TCNT1 = TIMER1_PRELOAD;

    TIMSK2 = 0;                        //Timer2 interrupts disable
    TCCR2A = 0;
    TCCR2B = 0 b00000101;              //prescaler: 16M/128=125000
    OCR2A = TIMER2_2K;
    ...
    TIMSK1 = 0 b00000001;              //разрешаем прерывание по переполнению Timer1
    EICRA = 0 b00000010;               //настраиваем внешнее прерывание по спаду импульса на INT0
    EIMSK = 0 b00000001;               //разрешаем внешнее прерывание INT0
}
...
ISR(INT0_vect)                         //внешнее прерывание на пине INT0 - считаем импульсы от счетчика
{
    ...
    if (++rad_sum > 999999UL * 3600 / GEIGER_TIME)
        rad_sum = 999999UL * 3600 / GEIGER_TIME;        //сумма импульсов

    conv_pump();                       //подкачка преобразователя

    if (buzz_disable == 0)
        buzz_pulse(buzz_vol);          //щелчок пищалкой
}
